...
<table>      
        <tr>
            <xsl:choose>
                <xsl:when test="string-length(@name) &gt; 0">
                    ...
                    <!--andrijic fix for multiple xsd files import-->   
                        <xsl:variable name="part-type"
                            select="document(//xsd:import[@schemaLocation]/@schemaLocation)//xsd:element[@name=$type-name and not(xsd:simpleType)][1] |
                            //xsd:element[@name=$type-name and not(xsd:simpleType)][1]"/>
        ...
        <xsl:choose>
            <xsl:when test="string-length($recursion.test) != 0">
	            ...
	            <xsl:choose>
	              ...
	              <xsl:otherwise>
	                  <!--andrijic added for imported xsd files-->
	                  <xsl:variable name="complex-type"
	                      select="document(//xsd:import[@schemaLocation]/@schemaLocation)//xsd:complexType[@name=$type-name]
	                      | //xsd:complexType[@name=$type-name] |
	                      document(//xsd:import[@schemaLocation]/@schemaLocation)//xsd:element[@name=$type-name]
	                      | //xsd:element[@name=$type-name]"/>
	                  <xsl:apply-templates select="$complex-type" mode="operations.message.part">
	                      <xsl:with-param name="anti.recursion" select="concat($anti.recursion, $recursion.label)"/>
	                  </xsl:apply-templates>
	              </xsl:otherwise>
	           </xsl:choose>
    ...
    <xsl:template match="xsd:element | xsd:attribute" mode="operations.message.part">
        ...
        <tr>
            ...
            <!--andrijic fix for multiple xsd imports and netsted import-->
            <!--xsl:variable name="elem-type"
                                     select="$consolidated-xsd[@name = $type-name and contains(local-name(), 'Type')][1]"/-->
            
            <xsl:variable name="elem-type"
                select="document(//xsd:import[@schemaLocation]/@schemaLocation)//*[@name=$type-name and contains(local-name(), 'Type')][1] |
                document(.)//*[@name = $type-name and contains(local-name(), 'Type')][1]"/>
    ...
    <xsl:template name="operations.render">
        <div class="page">
            ...
            <ul>
                ...
                <!--andrijic
                <xsl:apply-templates select="document(document(.)//xsd:import[@schemaLocation]/@schemaLocation)" mode="operations.message.part">                
                </xsl:apply-templates-->
            </ul>
        </div>
    </xsl:template>
